...
import { Check, X, MapPin, Users, DollarSign, Info, Loader2 } from "lucide-react";
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Admin Approval Panel</h1>
          <p className="text-gray-500 text-sm">Review travel plans and publish them to the platform.</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-gray-50/50 border-b border-gray-100">
              <tr>
                <th className="px-6 py-5 text-xs font-bold text-gray-400 uppercase tracking-widest">Destination</th>
                <th className="px-6 py-5 text-xs font-bold text-gray-400 uppercase tracking-widest">Capacity</th>
                <th className="px-6 py-5 text-xs font-bold text-gray-400 uppercase tracking-widest">Budget</th>
              ...
              {loading ? (
                <tr>
                  <td colSpan={5} className="py-20 text-center">
                    <Loader2 className="animate-spin mx-auto text-blue-500 mb-2" size={30} />
                    <span className="text-gray-400 text-sm font-medium">Fetching plans...</span>
                  </td>
                </tr>
              ) : plans.length === 0 ? (
                <tr>
                ...
                plans.map((plan) => (
                  <tr key={plan.id} className="hover:bg-blue-50/30 transition-all duration-200">
                    ...
                    <td className="px-6 py-4 text-gray-600">
                       <div className="flex items-center gap-2">
                        <Users size={16} className="text-gray-400" /> {plan.capacity} Person
                      </div>
                    </td>
                    <td className="px-6 py-4">
      ...
      <div className="mt-6 p-4 bg-gray-50 rounded-2xl flex items-start gap-3 border border-gray-100">
        ...
        <p className="text-sm text-gray-500 leading-relaxed">
          Once you click <b>Approve & Publish</b>, the travel plan will be visible to all tourists on the homepage. This action updates the plan status to `APPROVED`.
        </p>
      </div>
    </div>
  );
